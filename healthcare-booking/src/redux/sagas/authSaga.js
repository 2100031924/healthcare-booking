  ...
  try {
    ...
    if (!user) {
      const response = yield call(() =>
        fetch(`https://jsonplaceholder.typicode.com/users?email=${email}`).then((res) => res.json())
      );
...
function* handleForgotPassword(action) {
  try {
    ...
    yield put(forgotPasswordSuccess(`Password reset link sent to ${email}`));
  } catch (error) {
    yield put(forgotPasswordFailure('Failed to send reset link. Please try again.'));
  }
}
